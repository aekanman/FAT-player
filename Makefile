...
RUNTIME_ROOT_DIR := $(OBJ_ROOT_DIR)/runtime
...
ifneq ($(SOPC_NAME),)
CSV := $(ABS_QUARTUS_PROJECT_DIR)/$(SOPC_NAME)_tb.csv
endif # SOPC_NAME set
endif # ABS_QUARTUS_PROJECT_DIR set
endif # CSV == empty string

ifeq ($(MTI_TCL_SCRIPT),)
ifneq ($(CSV),)
MTI_TCL_SCRIPT := $(RUNTIME_ROOT_DIR)/simulation/mentor/$(notdir $(basename $(CSV)))/mti_setup.tcl
endif # CSV set
endif # MTI_TCL_SCRIPT == empty string
...
$(MTI_TCL_SCRIPT): $(CSV) $(MEM_INIT_DESCRIPTOR_FILE)
ifeq ($(CSV),)
	$(error No CSV file specified)
endif
	@$(MKDIR) $(@D)
	cd $(@D) && $(MTIGEN) -design_file=$(CSV) -device_compile -mem_init_file=$(ABS_MEM_INIT_DESCRIPTOR_FILE)

VSIM_COMMAND = \
	cd $(dir $(MTI_TCL_SCRIPT)) && \
	$(VSIM) -do "do $(notdir $(MTI_TCL_SCRIPT)); $(if $(VSIM_RUN_TIME),run ${VSIM_RUN_TIME};quit;)"

.PHONY: sim
sim: $(MTI_TCL_SCRIPT) mem_init_generate
ifeq ($(CSV),)
	$(error No CSV file specified)
endif
	$(VSIM_COMMAND)
